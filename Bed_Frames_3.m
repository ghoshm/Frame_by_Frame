...
%% Figure - Parameter Distributions V2 
figure; 
for p = 1:size(parameters,2) % For each parameter
    ...
    if ismember(p,[7:9 11:12]) == 0 % For most parameters 
       ...
       for e = 1:max(experiment_tags) % For each experimet 
        ...
        for g = 1:max(group_tags) % For each group
            for t = 1:2 % For day/night
                sample = size(find(data{2,t} == g & data{3,t} == e),1); 
                    % Find the number of samples for each window &
                        % experiment
                        % Sample = fish x time windows 
                
                legend_lines(col) = shadedErrorBar(1:crop,...
                    nanmean(data{1,t}(data{2,t} == g & data{3,t} == e,1:crop)),...
                    nanstd(data{1,t}(data{2,t} == g & data{3,t} == e,1:crop))/...
                    sqrt(sample),...
                    {'color',cmap_2(col,:)+(1-cmap_2(col,:))*(1-(1/e^.5))}); % Plot 
        ...
        axis([1 crop min(y_lims(2,:)) max(y_lims(1,:))]); % Set axis limits 
        xlabel(units(p),'Fontsize',12); % X labels 
        xl = xticklabels;  
        set(gca,'XTickLabels',cellstr(string(str2double(xl)/unit_conversion(1,p)))); 
            % Convert axis limits to appropriate units  
